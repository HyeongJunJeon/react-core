function createEffectStore() {
  ...
  /**
   * - cleanup 함수는 컴포넌트가 언마운트 될 때만 실행되는게 아니라
   *   *****effect가 다시 실행될 때 이전 effect(필요없어진 타이머나 이벤트 리스터)를 정리하기 위해 호출된다.*****
   * - 그래서 1. cleanup이 있다면 먼저 실행해서 이전 effect를 정리하고,
   *   2. 새 effect를 실행하고,
   *   3. 새로운 effect를 새로운 cleanup으로 저장하는 순서를 따라야한다.
   */
  function runEffects() {
  ...
  /**
   * 현재 컴포넌트가 App.js 하나라서 root를 강제로 비워서 cleanup을 테스트하기 위한 함수
   */
  function unmountApp() {
...
export const useEffect = effectStore.useEffect;
export const runEffects = effectStore.runEffects;
export const resetEffectIndex = effectStore.resetEffectIndex;
export const unmountApp = effectStore.unmountApp;
